...
#   - data/tdt_clean.csv             (TDT logistic fits: z, T50_prime, T50, per individual id)
#
# and attaches an Arrhenius fit (Ea_kJmol, lnA) of the Fv/Fm thermal-decay
...
#   2) set breadth_90 to NA if the 90%-of-Amax breadth extends beyond the
#      measured Tleaf range
#   3) set T50 to NA where the estimate falls outside 34-54 C
#   4) set z, T50_prime, Ea_kJmol, lnA to NA (all four, row-wise) if any
#      of the four is <0
#
# Individuals whose Schoolfield fit is entirely missing (Amax and T_opt
...
# ------------------------------------------------------------
# QC 1: delete curves where T_opt is fit outside the measured Tleaf range
...
topt_out_of_range <- faster %>%
  filter(!is.na(Tleaf_min), T_opt < Tleaf_min | T_opt > Tleaf_max) %>%
  pull(curveID)

if (length(topt_out_of_range) > 0) {
  message(
    "QC1: dropping ", length(topt_out_of_range), " curve(s) with T_opt outside ",
...
faster <- faster %>%
  filter(is.na(Tleaf_min) | (T_opt >= Tleaf_min & T_opt <= Tleaf_max))
exclusion_log[["QC1: T_opt outside measured Tleaf range (curve deleted)"]] <- length(topt_out_of_range)

# ------------------------------------------------------------
# QC 2: set breadth_90 to NA if the 90%-of-Amax breadth extends beyond
...
# fit_mod_SS.R used to originally compute breadth_90: rebuild the fitted
...
# >= 95% of Amax. That reproduces breadth_90 exactly; it's then compared
...
# ------------------------------------------------------------
recompute_breadth_bounds <- function(J_ref, E, E_D, T_opt, Amax, Tleaf_min, Tleaf_max) {
  if (any(is.na(c(J_ref, E, E_D, T_opt, Amax, Tleaf_min, Tleaf_max)))) {
    return(c(NA_real_, NA_real_))
  }
  T_extrap <- seq(Tleaf_min - 5, Tleaf_max + 5, by = 0.1)
  pred <- schoolfield(temp = T_extrap, J_ref = J_ref, E = E, E_D = E_D, T_opt = T_opt)
  above <- T_extrap[pred >= 0.95 * Amax]
  if (length(above) > 1) range(above) else c(NA_real_, NA_real_)
}

n_breadth_before <- sum(!is.na(faster$breadth_90))

faster <- faster %>%
  rowwise() %>%
  mutate(
    breadth_bounds = list(recompute_breadth_bounds(J_ref, E, E_D, T_opt, Amax, Tleaf_min, Tleaf_max)),
...
exclusion_log[["QC2: breadth_90 extends beyond measured Tleaf range (set NA)"]] <-
  n_breadth_before - sum(!is.na(faster$breadth_90))

# ------------------------------------------------------------
# 4. Arrhenius fit (Ea_kJmol, lnA) from the Fv/Fm thermal-decay time
# course in exp_all_wide.csv -- the same underlying dataset
# 00_tdt_fitting.R uses to fit T50/z, refit here as first-order decay
# rate k per individual x temperature, then Ea/lnA from the Arrhenius
...
arrhenius_fits <- k_estimates_clean %>%
  ...
  mutate(
    ...
    lnA      = purrr::map_dbl(fit, ~ coef(.x)[["(Intercept)"]])
  ) %>%
  select(id = plant_code, Ea_kJmol, lnA)
...
# ------------------------------------------------------------
# QC 4: if any of z, T50_prime, Ea_kJmol, lnA is <0 for a row, null out
# all four (not just the negative one)
# ------------------------------------------------------------
merged <- merged %>%
  mutate(
    .any_negative = purrr::pmap_lgl(
      list(z, T50_prime, Ea_kJmol, lnA),
      ~ any(c(...) < 0, na.rm = TRUE)
    ),
    z         = if_else(.any_negative, NA_real_, z),
    T50_prime = if_else(.any_negative, NA_real_, T50_prime),
    Ea_kJmol  = if_else(.any_negative, NA_real_, Ea_kJmol),
    lnA       = if_else(.any_negative, NA_real_, lnA)
  )

exclusion_log[["QC4: negative z/T50_prime/Ea_kJmol/lnA (row nulled)"]] <- sum(merged$.any_negative)

merged <- merged %>% select(-.any_negative)

# ------------------------------------------------------------
# Drop rows with no surviving faster curve: if Amax and T_opt are both
...
# against) "QC1: T_opt outside range" + "never had a faster curve" above
# -- those are the only two ways a row can end up missing Amax/T_opt.
# ------------------------------------------------------------
n_before_curve_filter <- nrow(merged)

merged <- merged %>%
  filter(!is.na(Amax) & !is.na(T_opt))

n_dropped_no_curve <- n_before_curve_filter - nrow(merged)
exclusion_log[["Total rows dropped for missing Amax/T_opt (= QC1 + never-had-curve, check)"]] <-
  n_dropped_no_curve
